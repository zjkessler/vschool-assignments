...
app.controller('myCtrl', ['$scope', '$http', function ($scope, $http) {
	...
	$scope.delete = function (todoToDelete, index) {
		...
		$http.delete('http://api.vschool.io/zjk/todo/' + todoToDelete._id).then(function (response) {
			response.data;
			$scope.allList.splice(index, 1);
		})
	};

	$scope.updateTodo = function (toUpdate) {
		$http.put('http://api.vschool.io/<yourname>/todo/' + toUpdate._id).then(function (response) {
			response.data;
